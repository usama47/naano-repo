    ...
    return (
      <Card className="p-6">
        ...
        <p className="mt-2 text-sm text-muted">
          Sample brief <span className="font-mono">{mutation.data.data.id}</span> is ready
          with one UTM link per creator. This demo does not send invites or track visits.
        </p>
        ...
        <Button className="mt-6 mr-3" onClick={() => {
          const url = URL.createObjectURL(new Blob([JSON.stringify(mutation.data, null, 2)], { type: "application/json" }));
          const anchor = document.createElement("a");
          anchor.href = url;
          anchor.download = "naano-sample-brief.json";
          anchor.click();
